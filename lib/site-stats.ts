...
/**
 * Installations counter: starts at a two-digit count on launch day and
 * climbs by a fixed amount every day after, computed from the calendar
 * date so every visitor (and the server) sees the same number without
 * needing a redeploy or client-side polling.
 */
const COUNTER_LAUNCH_DATE = new Date("2026-09-07T00:00:00Z");
const COUNTER_BASE_COUNT = 68;
